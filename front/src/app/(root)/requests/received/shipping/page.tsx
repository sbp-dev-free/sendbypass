...
const inboxShippingRequestsGetParams: RequestGet = {
  side: RequestSideSchema.Enum.RECEIVED,
  type: RequirementTypeEnum.Enum.SHIPPING,
};

const ReceivedShippingRequestsPage = () => {
  const {
    ...
    url: URLS.requests(),
    config: {
      params: inboxShippingRequestsGetParams,
    },
    isProtected: true,
  });

  const sortedRequests = requestsResponse?.results.sort((a, b) => {
...
export default ReceivedShippingRequestsPage;
